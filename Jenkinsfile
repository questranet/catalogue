pipeline {
  ...
  stages {
    ...
    stage('Release'){
      ...
      steps {
        sh 'zip -r backend-${TAG_NAME}.zip package.json server.js'
        sh 'curl -sSf -u "admin:Admin123" -X PUT -T catalogue-${TAG_NAME}.zip "http://artifactory.waleapagun.online:8081/artifactory/catalogue/catalogue-${TAG_NAME}.zip"'
      }
  //    steps{
